...
$query = $db->execute("SELECT * FROM `servers_info` LEFT JOIN `user_info_cache` ON `user_info_cache`.`steamid`=`servers_info`.`owner` WHERE `deleted` = 0 $where ORDER BY `servers_info`.`server_id`") or die($db->error());
